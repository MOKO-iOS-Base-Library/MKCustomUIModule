...
@property (nonatomic, strong)MKDFUModule *dfuModule;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_updatePage_dfuStart)]) {
        [self.delegate mk_updatePage_dfuStart];
    }
    ...
    @weakify(self);
    [self.dfuModule updateWithPeripheral:self.peripheral fileUrl:filePath progressBlock:^(CGFloat progress) {
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_updatePage_dfuProgress:)]) {
            [self.delegate mk_updatePage_dfuProgress:progress];
        }
    } sucBlock:^{
        @strongify(self);
        [[MKHudManager share] showHUDWithTitle:@"Update firmware successfully!" inView:self.view isPenetration:NO];
        [self performSelector:@selector(updateSuccess) withObject:nil afterDelay:3.f];
...
- (void)updateSuccess {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_updatePage_dfuSuccess)]) {
        [self.delegate mk_updatePage_dfuSuccess];
    }
...
- (void)updateFailed:(NSError *)error {
    self.leftButton.enabled = YES;
    [[MKHudManager share] hide];
    if ([self.delegate respondsToSelector:@selector(mk_updatePage_dfuFailed:)]) {
        [self.delegate mk_updatePage_dfuFailed:error];
    }
    [self.navigationController popToRootViewControllerAnimated:YES];
}
...
- (MKDFUModule *)dfuModule {
    if (!_dfuModule) {
        _dfuModule = [[MKDFUModule alloc] init];
    }
    return _dfuModule;
}
